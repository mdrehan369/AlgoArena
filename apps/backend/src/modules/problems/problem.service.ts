...
export class ProblemService {
  ...
  async getAllProblems(
    ...
    if (!id || id == "")
      throw new Error("User ID is required to fetch problems with user status");
    ...
    return problems.map(
      (
        problem: Prisma.ProblemGetPayload<{
          include: { submittedResults: true };
        }>,
      ) => {
        let userStatus = "";

        const prblm = problem.submittedResults.filter(
          (res: SubmittedResult) => res.userId === id,
        );
        if (prblm.length == 0) userStatus = "not-attempted";
        else if (prblm.find((res: SubmittedResult) => res.isAccepted == true))
          userStatus = "solved";
