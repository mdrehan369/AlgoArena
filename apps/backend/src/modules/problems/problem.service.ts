import { Level, PrismaClient, Topic } from "@repo/db";
...
export class ProblemService {
    ...
    async getAllProblems(
        ...
        return problems.map(problem => {
            ...
            const prblm = problem.submittedResults.filter(res => res.userId === id);
            if (prblm.length == 0) userStatus = "not-attempted";
            else if (prblm.find(res => res.isAccepted == true)) userStatus = "solved";
