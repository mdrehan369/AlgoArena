...
export default function SubmitModal() {
  ...
  const { language, problem, code } = useAppSelector(state => state.problemPage)
  const [testCases, setTestCases] = useState<Outputs[]>([])

  const [currentTestCase, setCurrentTestCase] = useState(0)
  const [finalResult, setFinalResult] = useState<SubmittedResult | null>(null)
  ...
  const getStateConfig = (state: SubmissionState) => {
    switch (state) {
      ...
      case "completed":
        return {
          title: finalResult?.isAccepted ? "Accepted!" : "Not Accepted",
          description: finalResult?.isAccepted
            ? "Congratulations! Your solution passed all test cases."
            : "Your solution didn't pass all test cases. Keep trying!",
          icon: finalResult?.isAccepted ? <IconCheck size={24} /> : <IconX size={24} />,
  ...
  const stateConfig = getStateConfig(submit.isPending ? "running" : "completed")
  ...
  return (
    <Modal
      ...
      title={
        <Group>
          <Box style={{ color: stateConfig.color }}>{stateConfig.icon}</Box>
          <Text fw={600} c="white">
            Submitting Solution
          </Text>
        </Group>
    ...
    >
      <Stack gap="lg" p="md">
          ...
          <Group>
            <Box
              style={{
                ...
                animation: submit.isPending ? "pulse 2s infinite" : "none",
              }}
            >
              {stateConfig.icon}
            </Box>
        ...
        >
          {
            compilationError ?
              <Card
                style={{ backgroundColor: "rgba(239, 68, 68, 0.1)", padding: "6px", border: "0px solid #ff545f", color: "#ff545f" }}
              >
                {compilationError}
              </Card>
              :
                        ...
                        >
                          <Group gap="sm">
                            ...
                            <Text c="white" size="sm">
                              Test Case {testCase.testCase.id}
                            </Text>
                            {!submit.isPending && testCase.status !== "FAIL" && (
                              <Badge size="xs" variant="light" color={testCase.status === "PASS" ? "green" : "red"}>
                                {testCase.status.replace(/_/g, " ").toLowerCase()}
                              </Badge>
                            )}
                          </Group>
                            ...
                            <Group gap="md">
                              <Group gap="xs">
                                ...
                                <Text c="gray.4" size="xs">
                                  {testCase.runtime.toFixed(1)}ms
                                </Text>
                              </Group>
                              <Group gap="xs">
                                <IconMeteor size={12} color="#6b7280" />
                                <Text c="gray.4" size="xs">
                                  {testCase.memory.toFixed(1)}MB
                                </Text>
                              </Group>
                            </Group>
                          )}
        ...
        {/* Final Result */}
        {finalResult && (
          <Transition mounted={!submit.isPending} transition="slide-up" duration={500}>
            {(styles) => (
              <Card
                padding="lg"
                style={{
                  ...styles,
                  backgroundColor: finalResult.isAccepted ? "rgba(16, 185, 129, 0.1)" : "rgba(239, 68, 68, 0.1)",
                  border: finalResult.isAccepted ? "1px solid #10b981" : "1px solid #ef4444",
                }}
              >
                <Stack gap="md">
                  <Group justify="center">
                    ...
                    >
                      {finalResult.isAccepted ? <IconTrophy size={32} /> : <IconX size={32} />}
                    </Box>
                  </Group>

                  <Text c="white" fw={700} size="xl" ta="center">
                    {finalResult.isAccepted ? "Solution Accepted!" : "Solution Not Accepted"}
                  </Text>
                  ...
                  <Group justify="center" gap="xl">
                    <Stack align="center" gap="xs">
                      ...
                      <Text c="white" fw={600}>
                        {finalResult.testCasesPassed}/{testCases.length}
                      </Text>
                    </Stack>
                    <Stack align="center" gap="xs">
                      <Text c="gray.4" size="sm">
                    ...
                    <Stack align="center" gap="xs">
                      ...
                      <Text c="white" fw={600}>
                        {finalResult.memory.toFixed(2)}MB
                      </Text>
                    </Stack>
                  </Group>
        ...
        {/* Action Button */}
        {!submit.isPending && (
          <Button fullWidth color="teal" size="md" onClick={() => dispatch(setIsSubmiting(false))}>
            {finalResult?.isAccepted ? "Continue Coding!" : "Try Again"}
          </Button>
        )}
      </Stack>
