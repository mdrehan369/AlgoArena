...
import { ProfileUpdateSchema } from '@/schemas/profile/profile.put.js'
...
} from '@/schemas/profile/profile.get.js'

const pump = promisify(pipeline)

export const profileController: FastifyPluginCallback = (instance, opts, done) => {
    ...
    fastify.put(
        ...
        async (
            ...
            const response = await fastify.profileService.updateProfile(fastify.user!.id, req.body)
    ...
    fastify.put('/picture', async (req: FastifyRequest, reply: FastifyReply) => {
        ...
        if (!file) return reply.status(400).send({ success: false, message: 'File not found' })
        ...
        if (fastify.user!.fileId) {
            const deleteResponse = await fastify.imageKitUtil.deleteImage(fastify.user!.fileId)
    ...
    fastify.get(
        ...
        async (req: FastifyRequest, reply: FastifyReply) => {
            const quick = await fastify.profileService.getQuickStats(fastify.user!.id)
            return reply.status(200).send({ success: true, data: quick, message: 'Fetched!' })
        }
    )
    ...
    fastify.get(
        ...
        async (req: FastifyRequest, reply: FastifyReply) => {
            const overview = await fastify.profileService.getStatsOverview(fastify.user!.id)
            return reply.status(200).send({ success: true, data: overview, message: 'Fetched!' })
        }
    )
    ...
    fastify.get(
        ...
        async (req: FastifyRequest, reply: FastifyReply) => {
            const recentSub = await fastify.profileService.getRecentSubmission(fastify.user!.id)
            return reply.status(200).send({ success: true, data: recentSub, message: 'Fetched!' })
        }
    )
