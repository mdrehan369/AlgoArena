...
export default async function spawner(
  spawningCmd: string,
  params: string[],
      ...
      result.on("close", (code, signal) => {
        ...
        if (stderrData && stderrData != "") {
          outputs.push({
            ...
            status: "FAIL",
            runtime,
            memory: Number(memoryPeak.toFixed(3)),
            console: stdoutData.replace("~", "\n"),
          });
          res(false);
          return;
        ...
        if ("output" in testCase && formattedStdOut == testCase.output)
          isPass = "PASS";

        outputs.push({
          testCase: testCase,
          output: formattedStdOut,
          status: isPass,
          runtime,
          memory: Number(memoryPeak.toFixed(3)),
          console: consoleOutput[0],
        });
        res(true);
      });
    });
