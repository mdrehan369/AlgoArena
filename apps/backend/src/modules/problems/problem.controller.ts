...
export const problemController: FastifyPluginCallback = (
  ...
  fastify.get(
    ...
    async (
      ...
      const problems = await fastify.problemService.getAllProblems(
        fastify.user.id,
        page,
        limit,
        status,
  ...
  fastify.get(
    ...
    async (
      ...
      if (!slug) {
        ...
        return reply
          .status(400)
          .send({ success: false, message: "No slug found" });
      }
      ...
      if (!response.success) {
        ...
        return reply
          .status(404)
          .send({ success: false, message: "No problem found!" });
      }
  ...
  fastify.post(
    ...
    async (
      ...
      if (!fastify.user) {
        return reply
          .status(400)
          .send({ success: false, message: "Authenticate first!" });
      }
      ...
      await fastify.kafkaProducer.producer.send({
        ...
        messages: [
          {
            key: id,
            value: JSON.stringify({ ...request.body, action: "SUBMIT" }),
          },
        ],
      });
