...
export default async function pythonExecutor(code: string, testCases: TestCase[], timeLimit: number, memoryLimit: number) {
  ...
  try {
    return await spawner(`${interpretor}`, [filename], testCases, timeLimit, memoryLimit)
  } catch (error: any) {
    console.log(error)
